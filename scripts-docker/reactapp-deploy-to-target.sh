...
sed "s/\${datadog_api_key}/$datadog_api_key/g" /home/ubuntu/jenkins-course/docker-compose-frontend.yml
...
if [[ -z $DOCKER_USERNAME || -z $DOCKER_PASSWORD || -z $SERVER_USERNAME || -z $SERVER_HOST || -z $datadog_api_key ]]; then
    ...
    cd /home/ubuntu/jenkins-course && docker-compose -f sudo docker-compose-frontend.yml --env-file <(echo "DD_API_KEY=${datadog_api_key}") up -d
